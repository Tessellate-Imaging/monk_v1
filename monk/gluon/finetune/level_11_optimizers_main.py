...
class prototype_optimizers(prototype_schedulers):
    ...
    ###############################################################################################################################################
    @warning_checks(None, ["lt", 1], momentum=["lt", 1.5], weight_decay=["lt", 0.01], post_trace=False)
    @error_checks(None, ["gt", 0], momentum=["gte", 0], weight_decay=["gte", 0], post_trace=False)
    @accepts("self", [int, float], momentum=[int, float], weight_decay=[int, float], post_trace=False)
    #@TraceFunction(trace_args=True, trace_rv=True)
