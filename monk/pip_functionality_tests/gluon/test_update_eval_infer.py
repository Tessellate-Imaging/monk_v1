...
def test_update_eval_infer(system_dict):
    ...
    if(forward):
        try:
            gtf.Prototype("sample-project-1", "sample-experiment-3", eval_infer=True);
            system_dict["successful_tests"] += 1;
            print_status("Pass");
        except Exception as e:
            system_dict["failed_tests_exceptions"].append(e);
    ...
    if(forward):
        try:
            gtf.update_model_path(["workspace/sample-project-1/sample-experiment-3/output/models/best_model-symbol.json",
