...
class finetune_model(finetune_dataset):
    ...
    def set_model_final(self, path=False):
        ...
        if(self.system_dict["model"]["params"]["model_path"]):
            ...
            if(os.path.isfile(self.system_dict["model_dir"] + 'final')):
                self.custom_print("    Loading model - {}".format(self.system_dict["model_dir"] + 'final'));
                self.system_dict["local"]["model"] = load_model(self.system_dict, final=True);
                self.system_dict = model_to_device(self.system_dict);
                self.custom_print("    Model loaded!");
                self.custom_print("");
            else:
                msg = "Model not found - {}\n".format(self.system_dict["model_dir"] + 'final');
                msg += "Previous Training Incomplete.";
                raise ConstraintError(msg);
        else:
            if(self.system_dict["states"]["resume_train"]):
                if(os.path.isfile(self.system_dict["model_dir"] + 'resume_state')):
                    self.custom_print("    Loading model - {}".format(self.system_dict["model_dir"] + 'resume_state'));
                    self.system_dict["local"]["model"] = load_model(self.system_dict, resume=True);
                else:
                    msg = "Model not found - \"{}\"\n".format(self.system_dict["model_dir"] + 'resume_state');
