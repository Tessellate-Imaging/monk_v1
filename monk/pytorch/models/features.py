...
class CNNVisualizer():
  ...
  def _extract_conv_layers(self):
    ...
    for name,layer in self.model.named_modules():
        if isinstance(layer, torch.nn.Conv2d):
          num_layers += 1
          conv_layers.update({name : layer})
          layer.register_forward_hook(self.visualize_hook)    

    print("Found {} convolutional layers".format(num_layers))
    return conv_layers

  def visualize_hook(self, layer, ip, op):
    self.fmaps.append(op)
  ...
  def get_layer_info(self, layer_name=None):
    ...
    filter_w = layer.weight.shape[2]
    filter_h = layer.weight.shape[3]
  ...
  def get_feature_maps(self, image_path, device):
  ...
  def visualize_kernel(self, layer_name='conv_1d',filter_id=0, channel_id=0, num_channels=1, cmap='gray', return_kernel=False):
    ...
    if filter_id >= layer.weight.shape[0] or channel_id >= layer.weight.shape[1]:
      return
    
    if num_channels == 1:
      kernel = kernels[filter_id][channel_id,:,:].detach()
    else:
      kernel = kernels[filter_id][channel_id:(channel_id+num_channels),:,:].detach()
    ...
    num_cols = min(4, int(layer.weight.shape[1]))
  ...
  def visualize_image(self, layer_name='conv_1d', channel_id=0, num_channels=1, cmap='gray', return_fmap=False):
    ...
    if channel_id >= layer.weight.shape[0]:
        return
      
    layer_id = list(self.conv_layers.keys()).index(layer_name)

    if num_channels == 1:
      fmap = self.fmaps[layer_id][0][channel_id,:,:].detach()
    else:
      fmap = self.fmaps[layer_id][0][channel_id:(channel_id+num_channels),:,:].detach()

    if return_fmap:
      return fmap

    num_rows = int(num_channels/4)
    num_cols = min(4, layer.weight.shape[0])
  ...
  def handle_layer_change(self,change):
    ...
    self.filters.max = (layer.weight.shape[0]-1)
    self.channels.max = (layer.weight.shape[1]-1)
  ...
  def visualize_kernels(self, store_path=None):
    ...
    self.filters = widgets.IntSlider(value=0, min=0, max=(layer.weight.shape[0]-1), step=1, description='Filter')
    self.channels = widgets.IntSlider(value=0, min=0, max=(layer.weight.shape[1]-1), step=4, description='Channel')
  ...
  def visualize_feature_maps(self, image_path, device='cpu', store_path=None):
    ...
    self.get_feature_maps(image_path, device)
    ...
    self.filters = widgets.IntSlider(value=0, min=0, max=(layer.weight.shape[0]-1), step=4, description='Filter')
    self.cmap = widgets.Dropdown(options=['viridis','gray'], value='gray', description='CMAP')
